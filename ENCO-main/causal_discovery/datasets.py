...
class ObservationalCategoricalData(data.Dataset):
    ...
    def __init__(self, graph, dataset_size):
        ...
        if not hasattr(self.graph, "data_obs"): # If it has the attribute "data_obs", we use it as the dataset Otherwise, a new dataset is sampled.
            ...
            print('其中，每一列代表每个随机变量的采样值，每一列分别对应第一行print的随机变量名')
            # print(np.shape(data)) data的每一列指的是该随机变量产生的观测值
