...
class CausalVariable(object):
    ...
    def __init__(self, name, prob_dist):
        ...
        prob_dist : ProbDist （实际上我们例子中是CategoricalDist，是ProbDist的子类，能称为ProbDist）
                                （可以查阅在定义这张图时，定义的那个概率函数，慢慢往上找）
                    Object representing the conditional probability distribution of the variable.
        """
        super().__init__()  #可是他并没有上个要继承的父类
        self.name = name
...
class CausalDAG(object):
    ...
    def sample(self, interventions=None, batch_size=1, as_array=False):
        ...
        var_vals = []    # 对于每个随机变量，采样得到的数据都放在这里，按照我们第一行print的顺序，分别对应每个随机变量的取值
        for v_idx, var in enumerate(self.variables):
            # 看样子似乎应该是先产生根节点的数据， 然后再产生它直接子代的数据， 以此类推产生后面的随机变量数据，
            # 后面产生数据的时候，他得需要根据它条件分布和父节点已经采样得到的数据进行数据生成。
            parents = np.where(self.adj_matrix[:, v_idx])[0]
            # print(self.adj_matrix)
            print('现在是到达了第', v_idx, '个节点', '这个随机变量的名字是', var, '它的父节点编号为：', parents)

            # print(np.where(self.adj_matrix[:, v_idx]))
            parent_vals = {self.variables[i].name: var_vals[i] for i in parents}  # 后面可以往这个父节点对应的采样值var_vals[i]中添加采样值
            # print(parent_vals)

            if interventions is None or (var.name not in interventions):  # No intervention
                # print('现在是到达了第', v_idx, '个节点')
                # print(parent_vals)

                sample = var.sample(parent_vals, batch_size=batch_size)

                # print('看看这个采样sample 的东西',sample) 根据每个随机变量的条件分布，输出自己取值概率最大的那个分类值
                # print('看看这个采样sample 的东西',np.shape(sample))
                # print('道路1') 我们运行的那个例子中，产生观测数据的时候，从这里走
                ...
                # print('道路3') 我们运行的那个例子中，产生干预数据的时候，从这里走
            else:  # Direct value assignment
                sample = interventions[var.name]
                # print('道路4') 我们运行的那个例子中没有从这里走
...
# num_vars 变成了 CausalDAG 的一个属性，代表这个因果图中随机变量的个数
