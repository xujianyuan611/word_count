...
def graph_from_adjmatrix(variable_names, dist_func, adj_matrix, latents=None):
    ...
    """
    variables = []
    print('目前的随机变量的顺序为', variable_names)
    for v_idx, name in enumerate(variable_names):
        parents = np.where(adj_matrix[:, v_idx])[0]

        #print(name,'的父母为：',parents)
        prob_dist = dist_func(input_names=[variable_names[p] for p in parents], name=name) #我知道为什么有警告，因为一开始没有默认值
...
def generate_categorical_graph(num_vars,
    ...
    # 应该是在定义每个随机变量的概率函数
    def dist_func(input_names, name): # 怎么能犯这种错误呢，这两个参数是定义的，当然可以在这里使用，我在这里先print指的是真正调用的时候的值。

        # 其中，name的条件概率输入的父母为input_names
        # dist_func的输入是父节点的name， prob_func的输入是父节点的name和数据组成的字典
        # print('让我们看看这个input_names是什么东西', input_names)
        print('现在是位于', name, '的地方')
        ...
        # print('我倒要看看你这个概率输出的是啥',dist)
        return dist

    
    return graph_func(variable_names, dist_func, **kwargs)

    #variable_names 在这个例子中指的就是 ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H']

    #    dist_func的定义 : (list[str],name) -> ProbDist
    #            Function for generating the probability distribution. It should take as input a
    #            list of the variables names representing the parents, and the name of the variable
    #            to model. As output, it is expected to give a ProbDist object.